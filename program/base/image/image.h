...
Vec3f Cimage::getColor(const float x, const float y, const int level) const{
  ...
  */
  ...
  // Do a tree sum reduction. 
  float p4[4];
  for(int i=0; i<4; i++) p4[i] = p0[i]+p1[i];
  float p5[4];
  for(int i=0; i<4; i++) p5[i] = p2[i]+p3[i];
  float p6[4];
  for(int i=0; i<4; i++) p6[i] = p4[i]+p5[i];
  return Vec3f(p6[0],p6[1],p6[2]);
  
  // Version I thought was vectorized
  //const unsigned char* ucp0 = &m_images[level][index];
  //const unsigned char* ucp1 = &m_images[level][index2];
  //// Load the four pixels
  //float p0[4] = {ucp0[0], ucp0[1], ucp0[2], 0.0f}; // * f00
